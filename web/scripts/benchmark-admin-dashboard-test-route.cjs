...
const TARGET_ROUTE = "/admin/dashboard-test";
...
async function main() {
  ...
  const warmup = await request(TARGET_ROUTE, { jar });
  if (warmup.status >= 400) {
    throw new Error(`Warmup failed: ${warmup.status}`);
  }

  const benchmark = await runConcurrent(100, async () => request(TARGET_ROUTE, { jar }));
  const payload = { route: TARGET_ROUTE, concurrent100: benchmark };

  const outputPath = path.join(__dirname, "..", "artifacts", "dashboard-test-benchmark.json");
